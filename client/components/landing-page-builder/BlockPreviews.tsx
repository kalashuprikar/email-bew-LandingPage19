...
export const TextHeadingsBlockPreview: React.FC<BlockPreviewProps> = ({
  block,
  isSelected,
  onSelect,
  onUpdate,
  ...
  const handleSaveElement = () => {
    ...
    const updatedElements = props.elements.map((el: any) =>
      el.id === editingElementId ? editValues : el
    );

    onUpdate({ ...props, elements: updatedElements });
  ...
  const handleAddElement = (type: string) => {
    const newElement = {
      id: `element-${Date.now()}`,
      type,
      text: type === "heading" ? "New Heading" : type === "quote" ? "Quote text" : "New text",
      level: type === "heading" ? "h2" : undefined,
      fontSize: type === "heading" ? "2rem" : type === "quote" ? "1.5rem" : "1rem",
      fontWeight: type === "heading" ? "bold" : "normal",
      textColor: type === "quote" ? "#1f2937" : "#4b5563",
    ...
    onUpdate({
      ...props,
      elements: [...props.elements, newElement],
    });
  };

  const handleDeleteElement = (elementId: string) => {
    const updatedElements = props.elements.filter((el: any) => el.id !== elementId);
    onUpdate({ ...props, elements: updatedElements });
  };

  const handleMoveElement = (elementId: string, direction: "up" | "down") => {
    const index = props.elements.findIndex((el: any) => el.id === elementId);
    if ((direction === "up" && index === 0) || (direction === "down" && index === props.elements.length - 1)) {
      return;
    }
    ...
    onUpdate({ ...props, elements: newElements });
  };
  ...
  return (
    <div
      ...
      style={{
        backgroundColor: props.containerBackgroundColor || "#ffffff",
        padding: props.containerPadding || "24px",
        margin: props.containerMargin || "0px",
        borderRadius: props.containerBorderRadius || "0px",
        boxShadow: props.containerBoxShadow || "none",
      }}
    >
      {/* Elements List */}
      <div className="space-y-4">
        {props.elements && props.elements.length > 0 ? (
              ...
              {editingElementId === element.id ? (
                <div className="space-y-2">
                  <textarea
                    ...
                    className="w-full p-2 focus:outline-none bg-white border border-orange-200 rounded resize-none overflow-hidden"
                    style={{
                      color: editValues.textColor || "#1f2937",
                      fontSize: editValues.fontSize || "1rem",
                      fontWeight: editValues.fontWeight || "normal",
                      fontFamily: editValues.fontFamily || "Arial, sans-serif",
                      textAlign: (editValues.textAlign || "left") as any,
                      lineHeight: editValues.lineHeight || "1.6",
                    }}
                    autoFocus
                    rows={3}
                  />
                  {element.type === "heading" && (
                    <select
                      value={editValues.level || "h2"}
                      onChange={(e) =>
                        setEditValues({ ...editValues, level: e.target.value })
                      }
                      className="w-full p-1 text-xs border border-gray-200 rounded focus:outline-none"
                    >
                  ...
                  {element.type === "heading" && (
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs bg-orange-100 text-orange-700 px-2 py-1 rounded">
                        {element.level?.toUpperCase() || "H2"}
                      </span>
                    </div>
                  )}
                  {element.type === "quote" && (
                    <blockquote className="italic pl-4 border-l-4 border-orange-400 ml-0">
                      "{element.text}"
                    </blockquote>
                  )}
                  {element.type !== "quote" && element.text}
                </div>
              )}

              {/* Element Controls - visible on hover */}
              {isSelected && editingElementId !== element.id && (
                <div className="absolute top-0 right-0 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 p-2">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
          ...
          <div className="text-center py-8 text-gray-400">No elements added yet</div>
        )}
      </div>

      {/* Add Elements Buttons */}
      {isSelected && editingElementId === null && (
        <div className="mt-6 pt-4 border-t border-gray-200">
          <p className="text-xs font-medium text-gray-600 mb-3">Add Element:</p>
          <div className="grid grid-cols-2 gap-2">
            <button
              onClick={(e) => {
                e.stopPropagation();
                handleAddElement("heading");
              }}
              className="p-2 text-xs border border-gray-300 rounded hover:bg-orange-50 hover:border-orange-300 transition-colors"
            >
              + Heading
            </button>
            <button
